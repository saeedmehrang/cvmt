...
def drop_extra_landmarks(
    ...
    if landmarks.shape[0] > CORRECT_V_LANDMARKS_SHAPE[shape_grp]:
        ...
        raise ValueError(
            "Number of annotated landmarks does not match the" " expected value!"
        )
    else:
        return landmarks
...
def sort_landmarks(
    ...
    if shape_grp == "2":
        ...
        dx, dy = (
            diffs[0],
            -diffs[1],
        )  # the negative sign is needed as y axis is reversed in images in python
        if dy / dx < 0:
...
def write_v_annots(
    ...
    for shape in v_annots["shapes"]:
        ...
        grp_shape.create_dataset(
            "points", data=points, compression="gzip", compression_opts=9
        )
    return True
...
def write_f_annots(
    ...
    f_annots_data_grp.create_dataset(
        "points", data=points, compression="gzip", compression_opts=9
    )
    return True
...
def write_image(
    ...
    image_data_grp.create_dataset(
        "data", data=image, compression="gzip", compression_opts=9
    )
    return True
...
def check_v_annots_integrity(
    ...
    try:
        if len(v_annots["shapes"]) != 3:
            raise ValueError(
                f"There are {len(v_annots['shapes'])} shapes in the v_annots!"
            )
    except Exception as e:
        print(e)
        invalid = True
...
def read_harmonized_hdf5(
    ...
    if "f_landmarks" in f.keys():
        f_landmarks = {"f_landmarks": f["f_landmarks"]["points"][:]}
    # read edges
    edges = f["edges"]["data"][:]
...
def write_edges(
    ...
    edges_data_grp.create_dataset(
        "data", data=edges, compression="gzip", compression_opts=9
    )
    return True
