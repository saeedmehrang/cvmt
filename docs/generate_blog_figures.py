...
landmarks = np.array([
    [128, 50], [135, 60], [142, 55],  # C2
    [120, 100], [128, 110], [136, 115], [144, 110], [152, 100],  # C3
    [115, 160], [125, 170], [135, 175], [145, 170], [155, 160]   # C4
])
...
fig.suptitle('Gaussian Heatmaps for 13 Vertebral Landmarks', fontsize=16, fontweight='bold')
...
axes[0].set_title('Distribution of Mean Radial Error on Validation Set', fontsize=14, fontweight='bold')
axes[0].legend()
...
for i, landmark in enumerate(landmarks):
    ax.add_patch(patches.Circle((landmark[0], landmark[1]), radius=4,
                                color='cyan', linewidth=2, fill=False))
    ax.text(landmark[0]+5, landmark[1], str(i), color='orange', fontsize=10,
            bbox=dict(boxstyle='round,pad=0.3', facecolor='black', alpha=0.7))

ax.set_title('Predicted Stage: CS3', fontsize=16, fontweight='bold', pad=20)
ax.text(0.5, 0.02, 'Bone Age Maturity Assessment Complete',
...
for ax, vert, title, color in zip(axes, vertebrae, titles, colors):
    ax.scatter(vert[:, 0], vert[:, 1], c=color, s=150, zorder=3, edgecolors='black', linewidth=2)
    ax.plot(vert[:, 0], vert[:, 1], color=color, alpha=0.5, linewidth=2, linestyle='--')
    for i, point in enumerate(vert):
        ax.annotate(str(i), (point[0], point[1]), fontsize=12, color='white',
...
plt.suptitle('Geometric Analysis Result: Stage CS3', fontsize=16, fontweight='bold', y=1.02)
plt.tight_layout()
