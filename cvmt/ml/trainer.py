...
class SingletaskTraining(pl.LightningModule):
    ...
    def _setup(self):
        if self.task_id == 1:
            ...
            raise ValueError(
                "The inserted `task_id` is incorrect. Accepted values are int 1 to 4."
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("train_mse", self.train_mse, on_step=True, on_epoch=True, logger=True)
    ...
    def configure_optimizers(self):
        ...
        if self.optim_params:
            optimizer = load_optimizer(
                **self.optim_params, model_parameters=self.parameters()
            )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val_mse", self.val_mse, prog_bar=True, logger=True)
        # check if task_id is either 3 or 4, then compute metric mre
        if self.task_id in [3, 4]:  # either v or f landmark detection
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.val_mse.update(preds, y)
        self.log("test_mse", self.val_mse, prog_bar=True, logger=True)
        # check if task_id is either 3 or 4, then compute metric mre
        if self.task_id in [3, 4]:  # either v or f landmark detection
            val_mre = mean_radial_error(preds, y)
...
def create_dataloader(
    ...
    train_file_list = [
        os.path.join(params.PRIMARY_DATA_DIRECTORY, file_path + ".hdf5")
    ...
    my_transforms = [
        transforms_mapping.get(t_name, **t_args)
        for t_name, t_args in transforms_config.items()
    ]
    ...
    if sampler_n_samples:
        ...
        sampler = RandomSampler(
            dataset, replacement=True, num_samples=sampler_n_samples
        )
        shuffle = False
    else:
        sampler = None
...
def trainer_edge_detection_single_task(
    params: EasyDict,
):
    # get the parameters
...
def max_indices_4d_tensor(inp_tensor: torch.Tensor):
    ...
    max_vals, max_inds = torch.max(
        inp_tensor.view(inp_tensor.shape[0], inp_tensor.shape[1], -1), dim=2
    )

    # Convert the indices into 2D coordinates
    max_inds_2d = torch.stack(
        (max_inds // inp_tensor.shape[3], max_inds % inp_tensor.shape[3]), dim=2
    )
